...
## 1. The provider roster, redefined
...
3. **Idology PASS, no sanctions flag → verified, exit.** (Unchanged — this path already works for
   84.8% of applicants.)
...
## 3. Stop conditions — where a user is hard-rejected, and why
...
## 4. Every change, tied to a Task 1 finding
...
| **3** | Persona IDV clears 86.3% of the failures it's tried on vs. 30.7% without it, but ran on under 1% of Idology failures (Task 1 §4.1) — the best tool, barely used. | Promoted to the standard second non-SSN fallback (L3) — but deliberately kept *after* the cheaper ACRO check, because it's also the highest-cost, highest-friction step (see §5). |
...
## 5. Cost, latency, and friction — explicit trade-offs
...
## 6. What's preserved, changed, and removed
...
| **Changed** | LexisNexis unified into one documented SSN-path role. Persona-SSN loses its unreproducible "legacy" gate. Sanctions/PEP becomes an always-checked, independently-routed flag rather than a signal folded into ordinary FAILs. Manual review triage is made a guarantee, not a possibility. |
| **Removed** | The 3.7% Idology-skip entry point (`ProviderA` acting as an undocumented primary). The unexplained "older accounts" Persona-SSN carve-out. |

---

## 7. Open items for Task 3
